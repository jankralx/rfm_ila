...
BWd2 = ceil(length(signal)/osf/2); % half of bandwidht in number of samples
chan_edges = ceil(length(signal)/osf*chan_edges);

Signal = fft(signal);

% calculate power in base channel
base_pwr = (sum(abs(Signal(1:1+BWd2)).^2) + ...
...
for i = 1:2:length(chan_edges)
    adj_pwr((i+1)/2,1) = (sum(abs(Signal(chan_edges(i):chan_edges(i+1))).^2));
    adj_pwr((i+1)/2,2) = (sum(abs(Signal(length(Signal)-chan_edges(i+1):...
